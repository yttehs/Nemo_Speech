...
```bash
cp -r /data/home/vishwas/Workspace/FastMSS/Data_French/multitalker_train_data Data_French/.
cp -r /data/home/vishwas/Workspace/FastMSS/Data_French/fastmss_final Data_French/.
```
...
python scripts/conf/generate_training_config.py \
    ...
    --train_cuts Data_French/multitalker_train_data/train_cuts.jsonl.gz \
    --dev_cuts Data_French/multitalker_train_data/dev_cuts.jsonl.gz \
    --test_cuts Data_French/multitalker_train_data/test_cuts.jsonl.gz \
    --exp_name multitalker_fr_cmltts_aws \
    --output conf/multitalker_finetune_overrides_aws_cmltts_fr.yaml \
    --batch_duration 90 \
...
python scripts/statistics/compute_overlap_stats.py \
    --cuts French_train=Data_French/multitalker_train_data/train_cuts.jsonl.gz \
    --cuts French_test=Data_French/multitalker_train_data/test_cuts.jsonl.gz \
    --cuts French_dev=Data_French/multitalker_train_data/dev_cuts.jsonl.gz
```
...
PYTHONPATH=. LD_LIBRARY_PATH="/opt/amazon/openmpi/lib:/usr/local/lib:/usr/lib" python scripts/train/train_multitalker_aws.py \
    ...
    --train_cuts Data_French/multitalker_train_data/train_cuts.jsonl.gz \
    --val_cuts Data_French/multitalker_train_data/dev_cuts.jsonl.gz \
    --test_cuts Data_French/multitalker_train_data/test_cuts.jsonl.gz \
