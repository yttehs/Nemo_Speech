...
```bash
cp -r /data/home/vishwas/Workspace/MagicData-RAMC/Data_Chinese/lhotse_cuts/*filtered.jsonl.gz Data_Chinese/multitalker_train_data/.
ln -s Data_Chinese/multitalker_train_data/dev_cuts_filtered.jsonl.gz Data_Chinese/multitalker_train_data/dev_cuts.jsonl.gz
ln -s Data_Chinese/multitalker_train_data/test_cuts_filtered.jsonl.gz Data_Chinese/multitalker_train_data/test_cuts.jsonl.gz
ln -s Data_Chinese/multitalker_train_data/train_cuts_filtered.jsonl.gz Data_Chinese/multitalker_train_data/train_cuts.jsonl.gz 
ln -s /data/home/vishwas/Workspace/MagicData-RAMC/Data_Chinese/chunked_audio Data_Chinese/.
```

## Generate the configuration yaml file
```bash
Note: We will have to generate the yaml file based on the data being used. Few hyperparamters need to be set after checking the data sample duration. Here we will use "md_cn" to represent MAGICDATA Chinese

python scripts/conf/generate_training_config.py \
    --template conf/multitalker_finetune_overrides_template.yaml \
    --train_cuts Data_Chinese/multitalker_train_data/train_cuts.jsonl.gz \
    --dev_cuts Data_Chinese/multitalker_train_data/dev_cuts.jsonl.gz \
    --test_cuts Data_Chinese/multitalker_train_data/test_cuts.jsonl.gz \
    --exp_name multitalker_md_cn_purity_lam025_aws \
    --output conf/multitalker_finetune_overrides_aws_purity_lam025_md_cn.yaml \
    --batch_duration 90 \
...
python scripts/statistics/compute_overlap_stats.py \
    --cuts French_train=Data_Chinese/multitalker_train_data/train_cuts.jsonl.gz \
    --cuts French_test=Data_Chinese/multitalker_train_data/test_cuts.jsonl.gz \
    --cuts French_dev=Data_Chinese/multitalker_train_data/dev_cuts.jsonl.gz
```

## Train the model
```bash
PYTHONPATH=. LD_LIBRARY_PATH="/opt/amazon/openmpi/lib:/usr/local/lib:/usr/lib" python scripts/train/train_multitalker_aws.py \
    --overrides conf/multitalker_finetune_overrides_aws_md_cn.yaml \
    --train_cuts Data_Chinese/multitalker_train_data/train_cuts.jsonl.gz \
    --val_cuts Data_Chinese/multitalker_train_data/dev_cuts.jsonl.gz \
    --test_cuts Data_Chinese/multitalker_train_data/test_cuts.jsonl.gz \
